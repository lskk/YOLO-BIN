...
def find_curvature(result_l,result_r):
    width_lanes = abs(result_l[0] - result_r[0])
    ym_per_pix = 30 / 720
    xm_per_pix = 3.7*(720/1280) / width_lanes

    lefty = np.array([result_l[1], result_l[3]])
    leftx = np.array([result_l[0], result_l[2]])
    righty = np.array([result_r[1], result_r[3]])
    rightx = np.array([result_r[0], result_r[2]])
    y_eval = result_l[1]

    '''
    lefty = np.linspace(result_l[1], result_l[3], num=10)
    leftx =  np.linspace(result_l[0], result_l[2], num=10)
    righty =  np.linspace(result_r[1], result_r[3], num=10)
    rightx =  np.linspace(result_r[0], result_r[2], num=10)
    '''
    ...
    left_x_mid = (result_l[0] +result_l[2] )/2
    left_y_mid = (result_l[1] +result_l[3] )/2
    right_x_mid = (result_r[0] +result_r[2] )/2
    right_y_mid = (result_r[1] +result_r[3] )/2

    left_fit_cr = np.polyfit(leftx * xm_per_pix,lefty * ym_per_pix, 2)
    right_fit_cr = np.polyfit(rightx * xm_per_pix, righty * ym_per_pix, 2)

    temp1 = (1.0 + (2.0*left_fit_cr[0]*left_x_mid+left_fit_cr[1])*(2.0*left_fit_cr[0]*left_x_mid+left_fit_cr[1]) )
    left_curvature = math.pow(temp1,1.5)/(2.0*left_fit_cr[0])

    temp2 = (1.0 + (2.0*right_fit_cr[0]*right_x_mid+right_fit_cr[1])*(2.0*right_fit_cr[0]*right_x_mid+right_fit_cr[1]) )
    right_curvature = math.pow(temp1,1.5)/(2.0*right_fit_cr[0])

    curvature = (left_curvature+right_curvature)/2
    '''
    return curvature

def road_info(result_l,result_r):
    """ print road information onto result image """
    curvature = find_curvature(result_l,result_r)
    direction = ((result_l[2]-result_l[0])+(result_r[2]-result_r[2]))/2
    ...
    center_lane = (result_r[0] + result_l[0] ) / 2
    lane_width = center_lane*2

    center_car = 720 / 2
    percent_deviation = abs(center_lane - center_car)/(lane_width / 2)*100
    percent_deviation = percent_deviation/1.8
    percent_deviation = round(percent_deviation, 3)
    if center_lane > center_car:
        deviation = 'Left ' + str(percent_deviation) + '%'
    elif center_lane < center_car:
        deviation = 'Right ' + str(percent_deviation) + '%'
    else:
        deviation = 'Center'
    
    return road_inf, curvature, deviation,percent_deviation

def print_road_status(img, result_l,result_r):
    """ print road status (curve direction, radius of curvature, deviation) """
    road_inf, curvature, deviation, percent_deviation = road_info(result_l,result_r)
...
def draw_fitline(img, result_l, result_r, color=(255, 0, 255), thickness=10):
    ...
    '''
    curv = find_curvature(result_l,result_r)
    direction = ((result_l[2]-result_l[0])+(result_r[2]-result_r[2]))/2
    print(curv)
    print(' ')
    print(direction)
    ...
    final =  print_road_status(final, result_l,result_r)
    return final
...
def detect_lanes_img(img):
    ...
    width_lanes = abs(left_fit_line[0] -  right_fit_line[0])
    xm_per_pix = 3.7*(720/1280) / width_lanes
    return final, xm_per_pix
